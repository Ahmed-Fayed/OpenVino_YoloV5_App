...
int main(){
    ...
    ov::Core core;
    // Step 2. Read a model
    std::shared_ptr<ov::Model> model = core.read_model("../../model/yolov5l6.xml");
    ...
    for(int i=1; i<=num_iters ;i++) {
        ...
        duration<double> elapsed_duration = duration_cast<duration<double>>(end_time - start_time);
        double elapsed_time = elapsed_duration.count();
        cout << "Inference time: " << elapsed_time << " seconds" << endl;
        average_time += elapsed_time;
    }

    average_time /= num_iters;
    cout<<"Average Inference Time = " << average_time <<endl;
